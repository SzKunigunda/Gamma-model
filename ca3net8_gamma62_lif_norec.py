...
J_PyrExc  = 0 # 0.15 in SPW # 0.02 gamma
...
p_rate_pyr = 1.0e-8 # input strenght
p_rate_bas = 1.0e-8
p_rate_sli = 1.0e-8
...
# Creating a range of exitory inputs

first = 10.0 
last = 35.0 
dataPoints = 20
exitations_mf = numpi.linspace(first, last, dataPoints)
...
for k in range (0, dataPoints):
	...
	filename_v_pyr = []
	for sp in range(NP):
		filename_spike_sub = "Results/%s_pyr%d_np%d_%s_norec.ras" % (run_name, sp, np, simulator_name) # output file for excit. population
		filename_spike_pyr.append(filename_spike_sub)
		filename_v_sub = "Results/%s_pyr%d_np%d_%s_norec.v" % (run_name, sp, np, simulator_name) # output file for membrane potential traces
		filename_v_pyr.append(filename_v_sub)
	filename_spike_bas  = "Results/%s_bas_np%d_%s_norec.ras" % (run_name, np, simulator_name) # output file for basket cell population  
	filename_spike_sli  = "Results/%s_sli_np%d_%s_norec.ras" % (run_name, np, simulator_name) # output file for slow inhibitory population
	filename_v_bas = "Results/%s_bas_np%d_%s_norec.v"   % (run_name, np, simulator_name) # output file for membrane potential traces
	filename_v_sli = "Results/%s_sli_np%d_%s_norec.v"   % (run_name, np, simulator_name) # output file for membrane potential traces
...
ax2.plot(exitations_mf, X[13, :], label='Gamma freq (exc.)', color='blue', linewidth=2, marker='o')
ax3 = ax2.twinx()
ax3.plot(exitations_mf, X[14, :], label='Gamma power (exc.)', color='red', linewidth=2, marker='|')
...
ax4.plot(exitations_mf, X[17, :], label='Gamma freq (inh.)', color='green', linewidth=2, marker='o')
ax5 = ax4.twinx()
ax5.plot(exitations_mf, X[18, :], label='Gamma power (inh.)', color='red', linewidth=2, marker='|')
...
fig2.tight_layout()
fig2.savefig('gammapower_exitation_range%.2fhz-%.2fhz_norec.png' %(first,last))
