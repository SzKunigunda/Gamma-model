...
J_PyrExc  = 0 # 0.15 in SPW  # kinullazzuk, es nem lesz rekurrens # 0.02 gamma
...
p_rate_pyr = 1.0e-8 # bemenet erossege
p_rate_bas = 1.0e-8
p_rate_sli = 1.0e-8
...
for k in range (0, dataPoints):
	...
	if meanEr > numpi.power(10,-4):
		avgGammaFE, gammaPE = gamma(fE, PxxE)
	else:
		avgGammaFE, gammaPE = (numpi.nan,numpi.nan)
	
	meanIr, rIAC, maxIAC, tMaxIAC, maxIACR, tMaxIACR, fI, PxxI, avgRippleFI, ripplePI = ripple(Bas_avg_rate, 1000)
	if meanIr > numpi.power(10,-4):
		avgGammaFI, gammaPI = gamma(fI, PxxI)
	else:	
		avgGammaFI, gammaPI = (numpi.nan,numpi.nan)
	...
	ax.set_title('Pyr. population rate %d | Mf exitation = %d' % (k,p_rate_mf))
	...
	ax6.set_title('Bas. population rate %d | Mf exitation = %d' % (k,p_rate_mf))
	...
	fig1.tight_layout()
	fig1.savefig('gammaPSD_norec%d.png' %k)
...
ax5.set_ylim([0, 100])
ax4.set_xlabel('scale factors')
ax4.legend(loc=2)
ax5.legend()

fig2.tight_layout()
fig2.savefig('gammapower_norec.png')
